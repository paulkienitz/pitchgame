    ...
    public function getRecentModerationRequests(): ?array
    {
        try
        {
            ...
            do {
                ...
                // when moderating suggestions we put the actual words into $rq->p->c just to have fewer params
            ...
            $getHistory = new Selector($this->marie, $this->log, 'iii', '
                WITH ide AS ( SELECT when_suggested AS when_posted, cast(unix_timestamp(when_suggested) AS double) AS when_posted_unix,
                                ...
                                FROM suggestions                   INNER JOIN
                                     ...
                                     objects  o USING (object_id)
                               WHERE session_id = ? ),
                     pit AS ( SELECT p.when_submitted AS when_posted, cast(unix_timestamp(p.when_submitted) AS double) AS when_posted_unix,
                                     NULL AS suggestion_id, subject_id, verb_id, object_id,
                                     ...
                                     p.shown_ct AS p_shown_ct, p.moderation_flag_ct AS p_flag_ct,
                                     p.moderation_status AS p_mod_status, p.is_deleted AS p_is_deleted,
                                     NULL AS moderation_id, NULL AS accepted_by, NULL AS rejected_by, NULL AS rejected_name
                                FROM pitches  p                    INNER JOIN
                                     subjects s USING (subject_id) INNER JOIN
                                     verbs    v USING (verb_id)    INNER JOIN
                                     objects  o USING (object_id)
                               WHERE session_id = ? ),
                     mor AS ( SELECT m.when_submitted AS when_posted, cast(unix_timestamp(m.when_submitted) AS double) AS when_posted_unix,
                                     NULL AS suggestion_id, ifnull(ps.subject_id, s.subject_id) AS subject_id,
                                     ifnull(pv.verb_id, v.verb_id) AS verb_id, ifnull(po.object_id, o.object_id) AS object_id,
                                     ifnull(ps.word, s.word) AS subject, ifnull(pv.word, v.word) AS verb, ifnull(po.word, o.word) AS object,
                                ...
                                FROM moderations m                                   LEFT JOIN
                                     ...
                                     objects     o  ON m.object_id   = o.object_id   LEFT JOIN
                                     pitches     p  ON m.pitch_id    = p.pitch_id    LEFT JOIN
                                     subjects    ps ON p.subject_id  = ps.subject_id LEFT JOIN
                                     verbs       pv ON p.verb_id     = pv.verb_id    LEFT JOIN
                                     objects     po ON p.object_id   = po.object_id
                               WHERE m.session_id = ? )
                ...
                WITH sesns AS ( SELECT session_id, nickname, signature, when_last_used, when_last_reviewed
                     ...
                     words AS ( SELECT session_id, sum(s.is_deleted + v.is_deleted + o.is_deleted) AS word_deletions,
                                  ...
                                  FROM sesns INNER JOIN
                                       ...
                                       verbs       v USING (verb_id)    INNER JOIN
                                       objects     o USING (object_id) 
                                 WHERE when_suggested >= ifnull(when_last_reviewed, DATE \'1901-01-01\') OR ? > 0
                                 GROUP BY session_id ),
                     ptchs AS ( SELECT session_id, sum(is_deleted) AS pitch_deletions, count(pitch_id) AS pitches
                                 ...
                                 WHERE when_submitted >= ifnull(when_last_reviewed, DATE \'1901-01-01\') OR ? > 0
                                 GROUP BY session_id ),
